...
const onSubmit = async (data) => {
    try {
      ...
      const response = await axios.post(`${API_BASE_URL}/user/registerWithCheck`, data);
      const { status } = response.data;
      if (status === "ok") {
        toast.success("Registration successful! Please log in.");
        setTimeout(() => {
          navigate("/login", { replace: true });
        }, 2000); // Wait for 5 seconds before navigating to login page
      } else {
        ...
        <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-xl">
          <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
            <form className="space-y-6" onSubmit={handleSubmit(onSubmit)}>
              <h2 class="mt-6 text-center text-3xl font-bold tracking-tight text-gray-900 p-5">
                Sign Up
              </h2>
